  ...
  return (
    <div className="flex justify-center content-center w-full">
      <Tabs className="w-full">
        <CustomTabList>
          <CustomTab>All Skills</CustomTab>
          <CustomTab>Frameworks</CustomTab>
          <CustomTab>CMS</CustomTab>
          <CustomTab>Styling</CustomTab>
          <CustomTab>State Management</CustomTab>
          <CustomTab>Back-End</CustomTab>
          <CustomTab>Communication Skills</CustomTab>
        </CustomTabList>

        <TabPanel className="mx-12 my-8 md:mx-8">
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-8 gap-x-10 md:gap-x-4 lg:gap-x-8">
            {skills.map((skill, i) => (
              <Skill skill={skill} />
            ))}
          </div>
...
const CustomTab = ({ children, ...otherProps }) => (
  <Tab {...otherProps}>
    <div className="truncate text-md fw-bold border-slate-900">{children}</div>
  </Tab>
);
...
const CustomTabList = ({ children, ...otherProps }) => (
  <TabList {...otherProps} className="justify-center py-2 tabsDiv">
    <div className="flex overflow-x-auto py-4 text-slate-700 justify-center content-center">
      {children}
    </div>
  </TabList>
);
